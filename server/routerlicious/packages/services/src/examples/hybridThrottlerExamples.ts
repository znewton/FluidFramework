...
export function createBasicHybridThrottler(
	...
	// Configure distributed throttling (existing pattern)
	...
	const distributedThrottler = new ThrottlerHelper(
		storageManager,
		100, // 100 operations per ms globally = 100,000 ops/sec
		10000, // Burst limit of 10,000 operations
		5000, // Check every 5 seconds (reduced from 16+ minutes!)
	);
...
export function createHighTrafficHybridThrottler(
	...
	const distributedThrottler = new ThrottlerHelper(
		storageManager,
		500, // 500 ops/ms = 500,000 ops/sec globally
		50000, // Large burst capacity
...
export function createLowLatencyHybridThrottler(
	...
	const distributedThrottler = new ThrottlerHelper(
		storageManager,
		50, // 50 ops/ms = 50,000 ops/sec globally
		5000,
	...
	return new HybridThrottler(distributedThrottler, localConfig, 1000);
}
...
export function migrateFromExistingThrottler(
	...
	const distributedThrottler = new ThrottlerHelper(
		...
		existingRatePerMs,
		existingBurstLimit,
		Math.min(existingMinInterval, 10000), // Cap at 10 seconds maximum
	);
	...
	return new HybridThrottler(
		distributedThrottler,
		localConfig,
		Math.min(existingMinInterval, 10000),
	);
}
